 ...
 */
export function webOriginOf(apiBase: string, webBase: string | undefined): string {
 ...
 */
export function htmlUrlOf(apiUrl: string, repo: string, type: string, webOrigin: string): string {
  try {
...
export function execGhToken(): Promise<string> {
...
export function githubErrorToWire(error: unknown): GithubError {
  ...
  const message = error instanceof Error ? error.message : String(error)
  return new GithubError('github-error', message, 502)
}
...
export class GithubInboxService {
  ...
  async merge(repo: string, pr: number, method: string): Promise<void> {
    if (!this.config.allowMerge) {
      throw new GithubError('github-forbidden', 'merge is disabled by configuration (githubAllowMerge)', 403)
    }
  ...
  private async requireClient(): Promise<GithubClient> {
    const resolved = await this.resolveToken()
    if (resolved.token === undefined) {
      throw new GithubError('github-unavailable', 'GitHub is not configured (no token resolved)', 503)
    }
